...
for line in `cat $file`
do
  echo -e "\"title\"***\"category\"***\"text\"
  \"$line dolmuş saatleri ve güzergahı\"***\"$city1 ulaşım\"***\"<b>$line dolmuş saatleri ve güzergahı</b> tarafımızca takip edilmekte ve bilgi verilmektedir. $city $line ulaşım bilgisine belediye otobüsü saatlerine ve dolmuş saatleri bulunmaktadır. Sitemiz üzerinde bulunan iller kısmından tüm <b>Türkiye</b>'deki ulaşım bilgilere ulaşabilirsiniz. <b>$city $line</b> için sitemiz tarafından bulunan tüm ulaşım bilgileri yayınlanmıştır. <a href="https://dolmussaatleri.com/$city1'_'$ulasim1/">$city $ulasim</a> sayfası üzerinden tüm ulaşım saatleri paylaşılmaktadır. <h2>$line dolmuş saatleri</h2> $city aktif olan tüm dolmuş saatleri aşağıda paylaşılmaktadır. [catlist name= \"$city dolmus\" numberposts=-1] Ulaşım $city için güncellenen bilgiler sitemiz üzerinden yayınlanacaktır. Dolmuş saatleri ve güzergahlarının tüm güncellemesi sitemizde paylaşılmıştır. Yeni yapılan değişiklikleri tarafımıza bildirmeniz durumunda gerçekliği kontrol edilip sitemiz güncellenecektir. <a href="https://dolmussaatleri.com/iletisim/">İletişim sayfamız</a> üzerinden bizi bilgilendirebilirsiniz. Ulaşım saatleri $city $line trafik durumunda göre değişiklik gösterebilir. Resmi ve dini bayramlarda ücretlerde aynı şekilde değişiklik göstermektedir. Ulaşım saatlerindeki değişiklikler $city hava koşuluna göse tekrar değişiklik göstermektedir. Gerekli ulaşım bilgileri tarafımızca takip edilmekte ve anlık olarak güncelleme yapılmaktadır.\"" >> ./sonuc/"$line.csv"
done
